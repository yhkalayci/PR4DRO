...
from dataloaders.civilcomments import CivilCommentsDataModule, CivilCommentsDataset
...
def civilcomments_pipeline():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Load the dataset
    data_module = CivilCommentsDataModule(root='data', batch_size=32)
    data_module.setup()
    ...
    torch.save(model.state_dict(), 'civilcomments_pretrain.pt')
    ...
    for ratio in ratios:
        ...
        finetuned_model = BertClassifier(num_classes=2).to(device)
        finetuned_model.load_state_dict(torch.load('civilcomments_pretrain.pt'))
        
        # Finetune the model
        finetune_bert_classifier(finetuned_model, subset_dataset, data_module.val_dataset, 
                                 num_iterations=10, learning_rate=2e-5, batch_size=32, 
                                 save_path=f"./civilcomments_finetune_{ratio}")
    ...
    model.load_state_dict(torch.load('civilcomments_pretrain.pt'))
    ...
    for ratio in ratios:
        model.load_state_dict(torch.load(f"./civilcomments_finetune_{ratio}/best_finetuned_model.pth"))
        avg_acc, worst_group_acc, group_accuracies = calculate_accuracies(model, test_loader, device)
        print(f"Finetuned Model (ratio {ratio}) - Average Accuracy: {avg_acc:.4f}, Worst-group Accuracy: {worst_group_acc:.4f}")
        print(f"Group Accuracies: {group_accuracies}")

if __name__ == "__main__":
    civilcomments_pipeline()